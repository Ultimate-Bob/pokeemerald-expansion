...
void Overworld_PlaySpecialMapMusic(void)
{
    ...
    if (music != MUS_ABNORMAL_WEATHER && music != MUS_NONE)
    {
        if (gSaveBlock1Ptr->savedMusic)
            ...
            music = GetLocationMusicSurf(gMapHeader.regionMapSectionId); // feature/location-battle-music
    }
...
static void TransitionMapMusic(void)
{
    ...
    if (FlagGet(FLAG_DONT_TRANSITION_MUSIC) != TRUE)
    {
        ...
        {
            if (currentMusic == MUS_UNDERWATER || currentMusic == GetLocationMusicSurf(gMapHeader.regionMapSectionId)) // feature/location-battle-music
                return;
            if (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING))
                newMusic = GetLocationMusicSurf(gMapHeader.regionMapSectionId); // feature/location-battle-music
        }
