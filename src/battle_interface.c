...
static void UpdateNickInHealthbox(u8 healthboxSpriteId, struct Pokemon *mon)
{
    ...
    side = IsOnPlayerSide(gSprites[healthboxSpriteId].data[6]) ? B_SIDE_PLAYER : B_SIDE_OPPONENT; // GetBattlerSide(gSprites[healthboxSpriteId].hMain_Battler);
    ...
    if (wild && shiny && side == B_SIDE_OPPONENT)
        windowTileData = AddTextPrinterAndCreateWindowOnHealthboxToFitWithColours(gDisplayedStringBattle, 0, 3, 9, 4, 2, &windowId, 55);
    else
...
{
    return AddTextPrinterAndCreateWindowOnHealthboxWithOptions(str, x, y, 1, 3, bgColor, windowId, fontId);
}
...
{
    u32 fontId = GetFontIdToFit(str, FONT_SMALL, 0, width);
    return AddTextPrinterAndCreateWindowOnHealthboxWithOptions(str, x, y, fgColor, shColor, bgColor, windowId, fontId);
}
