...
void UpdateNickInHealthbox(u8 healthboxSpriteId, struct Pokemon *mon)
{
    ...
    {
        // feature/shiny-wild-special-handling
        union TextColor textCol = (wild && shiny) ? sHealthBoxTextColorShiny : sHealthBoxTextColor;

        FillSpriteRectColor(healthboxSpriteId, 8, 5, 55, 11, HEALTHBOX_BG_INDEX);
        AddSpriteTextPrinterParameterized6(healthboxSpriteId, fontId, 8, 3, 0, 0, textCol, 0, gDisplayedStringBattle);
    }
